...
if uploaded_files:
    ...
    df["Rank"] = df["Rank"].astype(str)  # Stringe çevir
    df["Rank"] = df["Rank"].str.extract(r'(\d+)')  # Sadece rakamları al
    df["Rank"] = pd.to_numeric(df["Rank"], errors='coerce').fillna(250).astype(int)
